...
contract Treasury is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256[3] public distributeRates = [20, 20, 60]; // distribute rates index0 => ant coin, index1 => ant lp, index2 => assets
    // buy / sell tax rates
    ...
    uint256 public constant PRECISION_E6 = 1e6;

    address public _wETH;
    address public usdtAddress = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    address public antCToken;
    address public teamWallet;
    address public deadWallet1 = 0x000000000000000000000000000000000000dEaD;
    ...
    constructor(
        ...
        address antCAddress
    ) ERC20("KOA Treasury Token", "KOATT") {
    ...
    function getANTCoinLPTokenUSDValue(uint256 _lpTokenAmount) public view returns (uint256) {
        ...
        uint256[] memory amounts0 = uniswapV2Router.getAmountsOut(
            IERC20(_wETH).balanceOf(address(antCLPPair)),
            _path
        );

        uint256 totalETHValueOfPair = amounts0[1];
        uint256 totalValueOfPair = totalETHValueOfPair;
        uint256 lpTokenPriceForReturn = (totalValueOfPair * _lpTokenAmount) /
            circulatingSupply;
    ...
    function getLPTokenAmountFromUSDValue(uint256 _usdValue) public view returns (uint256) {
        ...
        uint256[] memory amounts1 = uniswapV2Router.getAmountsIn(
            _usdValue,
            _path
        );

        uint256 totalETHValueForReturn = amounts1[0];
        uint256 totalValueForReturn = totalETHValueForReturn;
        uint256 lpTokenAmountForReturn = (circulatingSupply * totalValueForReturn) / totalSupplyOfLPPair;
