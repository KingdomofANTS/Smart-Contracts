...
describe("BasicANT", function () {
    ...
    describe("Test Suite", function () {
        ...
        it("setBatchInfo: should work if caller is the owner", async () => {
            ...
            expect(expected1.toString()).to.be.equal("name1,testBaseURI1,0,1000," + ANTCoinContract.address + ",10000,true");
            expect(expected2.toString()).to.be.equal("name2,testBaseURI2,0,2000," + ANTCoinContract.address + ",20000,true");
        })
        ...
        it("downgradeLevel: should fail if Caller is not the owner or minter", async () => {
            await expect(BasicANTContract.connect(badActor).downgradeLevel(0, 1)).to.be.revertedWith("BasicANT: Caller is not the owner or minter");
        })
        ...
        it("ownerMint: should work if caller has a minter role", async () => {
            ...
            expect(batchInfo.toString()).to.be.equal("name1,testBaseURI1,2,1000," + ANTCoinContract.address + ",10000,true");
            const antInfo = await BasicANTContract.getANTInfo(1);
        ...
        describe("mint", () => {
            ...
            it("should work if all conditions are correct", async () => {
                ...
                expect(batchInfo1.toString()).to.be.equal("name1,testBaseURI1,3," + maticMintPrice + "," + ANTCoinContract.address + ",10000,true");
                ...
                expect(batchInfo2.toString()).to.be.equal("name2,testBaseURI2,5," + maticMintPrice + "," + ANTCoinContract.address + ",10000,true");
        ...
        describe("upgradeMint", () => {
            ...
            it("should work if all conditions are correct", async () => {
                ...
                expect(potionBalance1.sub(3)).to.be.equal(potionBalance2);
        ...
        it("transferFrom: should transfer token without approve step if caller has minter role", async () => {
            ...
            await BasicANTContract.setBatchInfo(0, "name1", "testBaseURI1", maticMintPrice, ANTCoinContract.address, tokenAmountForMint);
            await BasicANTContract.connect(user1).mint(0, user1.address, 2, { value: maticMintPrice * 2 });
            await expect(BasicANTContract.transferFrom(user1.address, BasicANTContract.address, 1)).to.be.not.reverted
        })
